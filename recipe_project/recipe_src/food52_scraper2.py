...
def scraper(titles, weblinks):
    ...
    # weblink = 'https://food52.com/recipes/11208-beef-stroganoff-with-mushrooms-and-onions'
        r = requests.get(link)
        soup = BeautifulSoup(r.content, 'html.parser')
        try:
            author = (soup.find('a', attrs={'itemprop':'author'})).text
            date_test = soup.find('p', attrs={'class':'article-header-meta meta'})
            date_string = "".join(
            ...
            clean_date = "".join(
                 ...
                 for idx, item in enumerate(date_string.split()) if item in dateset]
                )
            new_like_count = "".join(list(soup.find('span', attrs={'class':'counter'}).children)) # to get the count
            serving_search = soup.find('p', attrs={"itemprop":"recipeYield"})
            serves = serving_search.find('strong')
            servings = "".join([num for num in serves.text.split() if num.isdigit()])
